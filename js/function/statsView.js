...
function renderDonutChart() {
    ...
    const currentMonthExpenses = sharedState.entries.filter(entry => 
        entry.date.startsWith(currentMonthStr) && !entry.isIncome
    );
    
    if (currentMonthExpenses.length === 0) {
        donutChartContainer.innerHTML = '<div class="no-data">데이터가 없습니다.</div>';
        return;
    }
...
function renderExpenseSummary() {
    ...
    const monthTotalExpenseElement = document.getElementById('month-total-expense');
    ...
    if (currentMonthExpenses.length === 0) {
        expenseListContainer.innerHTML = '<div class="no-data">데이터가 없습니다.</div>';
