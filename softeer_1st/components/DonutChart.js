...
export function DonutChart({ data, total }) {
    ...
    data.forEach((item) => {
        const legendItem = createElement("div", {
            className: "donut-legend-item flex align-center gap-4",
            innerHTML: `
                <div class="colorchip" style="background-color: ${backgrounds[item.category]}">${item.category}</div>
                <div class="serif-14">${Math.round(item.amount / total * 100)}%</div>
                <div class="serif-14">${item.amount.toLocaleString()}원</div>
            `,
        });
