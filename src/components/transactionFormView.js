...
function createDateInput() {
  // date는 FormState가 우선, 없으면 DateState 사용
  const formState = FormState.getFormState();
  const date = formState.date || DateState.getDate();
  return `
...
export function createMethodSelect() {
  const formState = FormState.getFormState();
...
export function renderTransactionForm() {
  ...
  innerHTML += createDateInput();
  innerHTML += createAmountGroup();
  innerHTML += createDescription();
  innerHTML += createMethodSelect();
  innerHTML += createCategorySelect();
